...
void render_map_cell(s32 col, s32 row, char visibility)
{
    MATRIX cell_matrix;
    SVECTOR position;
    long flag;
    ...
    object_index = ((u8)(map_cell_attribute_grid.cells[row][col]));
    if (object_index == ((u8)(KF_MAP_ATTRIBUTE_NONE))) {
        return;
    }
    ...
    if (staff_timer != KF_ILLUSION_STAFF_INACTIVE
        ...
        switch (object_index) {
        case ((u8)(KF_MAP_ATTRIBUTE_44)):
            object_index = ((u8)(KF_MAP_ATTRIBUTE_17));
            break;
        case ((u8)(KF_MAP_ATTRIBUTE_HIDDEN_DOOR)):
            object_index = ((u8)(KF_MAP_ATTRIBUTE_18));
            break;
        case ((u8)(KF_MAP_ATTRIBUTE_46)):
            object_index = ((u8)(KF_MAP_ATTRIBUTE_19));
            break;
        }
    }
    ...
    orient = ((u8)(map_cell_orientation_grid.cells[row][col])) - 1;
    ...
    if (orient == ((u8)(KF_MAP_ORIENT_QUARTER_TURN)) - 1) {
        position.vz += KF_MAP_TILE_SIZE;
    } else if (orient == ((u8)(KF_MAP_ORIENT_HALF_TURN)) - 1) {
        position.vx += KF_MAP_TILE_SIZE;
        position.vz += KF_MAP_TILE_SIZE;
    } else if (orient == ((u8)(KF_MAP_ORIENT_THREE_QUARTER_TURN)) - 1) {
        position.vx += KF_MAP_TILE_SIZE;
    }

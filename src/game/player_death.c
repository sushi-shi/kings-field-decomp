...
void player_death_restart(void)
{
    ...
    if (map_runtime_state.world_state.floors[0].script.floor1.revival_enabled == KF_MAP_SCRIPT_SET
        && item_stock[((u8)(KF_ITEM_STOCK_PLAYER))]
...
void player_recalculate_combat_stats(void)
{
    ...
    if (player_state.base_magic >= DISPOISON_REQUIRED_BASE_MAGIC
        && effect_state.magic.entries[((u8)(KF_MAGIC_HEALING))].learned
            != KF_MAGIC_UNLEARNED
        && effect_state.magic.entries[((u8)(KF_MAGIC_DISPOISON))].learned
            == KF_MAGIC_UNLEARNED) {
        effect_state.magic.entries[((u8)(KF_MAGIC_DISPOISON))].learned = KF_MAGIC_LEARNED;
        notify_enqueue(KF_NOTIFICATION_MAGIC_LEARNED);
    }
    if (player_state.base_magic >= FIRE_WALL_REQUIRED_BASE_MAGIC
        && effect_state.magic.entries[((u8)(KF_MAGIC_FIRE_WALL))].learned
            == KF_MAGIC_UNLEARNED) {
        effect_state.magic.entries[((u8)(KF_MAGIC_FIRE_WALL))].learned = KF_MAGIC_LEARNED;
        notify_enqueue(KF_NOTIFICATION_MAGIC_LEARNED);
    }
    if (player_state.base_magic >= LIGHTNING_BOLT_REQUIRED_BASE_MAGIC
        && effect_state.magic.entries[((u8)(KF_MAGIC_LIGHTNING_BOLT))].learned
            == KF_MAGIC_UNLEARNED) {
...
void player_select_magic(KfEffectKind magic_id)
{
    ...
    if (magic_id == KF_MAGIC_NONE) {
        ...
        player_state.selected_magic_record =
            &effect_state.magic.entries[((u8)(player_state.selected_magic_id))];
    }
}

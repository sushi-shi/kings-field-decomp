...
enum {
    ...
    PROJECTILE_ROLL_STEP = 200,
    EMERGING_ROLL_STEP = 100,
    PROJECTILE_DEFAULT_ROLL_STEP = 600,
    EFFECT_DISSIPATE_SCALE_STEP = 400,
...
lightning_impact:
                    ...
                    effect->render_id.billboard = ((KfEffectBillboardId)(((u8)(effect->base_render_id.billboard)) + (effect->control.frames_remaining & 1)));
                }
                return;
            }
            if (kind == KF_EFFECT_KIND_SCATTER_PROJECTILE) {
                ...
                if (--effect->control.frames_remaining == 0) {
                    if (effect->propagation.generations_remaining != 0) {
                        ...
                        effect_pool_construct(
                            ...
                            (effect->propagation.generations_remaining), (effect->control.frames_remaining), ((s16)effect->scale_x));
        ...
        if (phase < KF_EFFECT_FIRE_BALL_IMPACT_END && kind == KF_MAGIC_FIRE_BALL) {
            effect->render_id.billboard = ((KfEffectBillboardId)(((u8)(effect->base_render_id.billboard)) + ((u8)(phase))));
...
play_phase_sound:
    ...
    case KF_EFFECT_KIND_MOONLIGHT_PROJECTILE:
        if (((u8)(phase)) < ((u8)(KF_EFFECT_MOONLIGHT_TRAVEL_LAST)) + 1) {
            if (effect_map_collision(&effect->position, PROJECTILE_COLLISION_RADIUS) != KF_COLLISION_NONE) {
                ...
                audio_play_spatial_default_range(
                    &magic_records[((u8)(KF_EFFECT_KIND_RADIAL_BLAST))].sounds[1], &effect->position, KF_AUDIO_MAX_VOLUME);
                return;
            }
            addVector(&effect->position, &effect->direction.vector);
            ...
            if (((((u8)(phase)) - ((u8)(KF_EFFECT_MOONLIGHT_IMPACT_FIRST))) & 1) == 0) {
                effect_pool_construct(
                    effect->id, effect->type, KF_EFFECT_KIND_RADIAL_BLAST,
                    &effect->position, &effect->direction.vector, (KF_EFFECT_SOUND_PLAY));
            }
    ...
    case KF_EFFECT_KIND_GROUND_TRAIL: {
        ...
        case KF_EFFECT_GROUND_TRAIL_WAIT_FOR_PARENT:
            if (((u8)(linked_effect->phase)) > ((u8)(KF_EFFECT_MOONLIGHT_IMPACT_FIRST)) - 1) {
        ...
        if (((u8)(effect->render_id.billboard)) >= ((u8)(effect->base_render_id.billboard)) + GROUND_TRAIL_RENDER_FRAME_COUNT) {
    ...
    case KF_EFFECT_KIND_RADIAL_BLAST:
        if (++effect->phase < KF_EFFECT_RADIAL_BLAST_PHASE_END) {
            ...
            damage_radius = ((u8)(phase)) * RADIAL_BLAST_RADIUS_STEP;
            power = effect_magic_power(effect);
            if (((u8)(effect->phase)) & 1) {
...
randomize_homing_direction:
    ...
    case KF_EFFECT_KIND_LIGHTNING_IMPACT:
        if (phase > KF_EFFECT_LIGHTNING_IMPACT_PHASE_LAST) {
            ...
            effect->render_id.billboard++;
            if (((u8)(effect->render_id.billboard)) >= ((u8)(effect->base_render_id.billboard)) + LIGHTNING_IMPACT_RENDER_FRAME_COUNT) {
                effect->render_id.billboard = effect->base_render_id.billboard;
            }
            if (phase == KF_EFFECT_LIGHTNING_IMPACT_EMIT_FIRST || phase == KF_EFFECT_LIGHTNING_IMPACT_EMIT_SECOND || phase == KF_EFFECT_LIGHTNING_IMPACT_EMIT_LAST) {
                ...
                if (phase == KF_EFFECT_LIGHTNING_IMPACT_EMIT_FIRST) {
                    phase_sound = &magic_records[((u8)(KF_MAGIC_LIGHTNING_BOLT))].sounds[1];
        ...
        if (phase > KF_EFFECT_LIGHTNING_BLAST_PHASE_LAST) {
            ...
            if (((u8)(phase)) & 1) {
                ...
                setVector(&position,
                    ...
                    effect->position.vz);
                damage_radius = ((u8)(phase)) * LIGHTNING_BLAST_RADIUS_STEP;
                power = effect_magic_power(effect);
                lightning_magic = &magic_records[((u8)(KF_MAGIC_LIGHTNING_BOLT))];
                actor_pool_apply_radial_damage(
...
advance_effect_phase:
    ...
    case KF_MAGIC_FIRE_WALL:
        if (phase < KF_EFFECT_GROUND_BRANCH_GROW_END) {
            ...
            if (((u8)(phase)) == ((u8)(KF_EFFECT_GROUND_BRANCH_GROW_END)) - 1) {
                switch (effect->propagation.branch) {
                case KF_EFFECT_GROUND_BRANCH_ROOT:
            ...
            if ((((u8)(phase)) & (GROUND_BRANCH_DAMAGE_PERIOD - 1)) == 0) {
    ...
    case KF_EFFECT_KIND_ACTOR_SPAWNER: {
        ...
        } else if (((u8)(phase)) < ((u8)(KF_EFFECT_ACTOR_SPAWNER_TRAVEL_LAST)) + 1) {
            VECTOR position;

...
KfMagicPanelResult menu_magic_panel(void)
{
    ...
    found = 0;
    for (code = ((s32)(KF_MAGIC_HEALING)); code < ((s32)(KF_MAGIC_LIGHTNING_BOLT)); code++) {
        if (magic_records[code].learned == KF_MAGIC_LEARNED) {
            for (j = 0; j < MENU_GLYPHS_PER_ROW; j++)
                labels[found][j] = magic_name_rows[code].codes[j];
            codes[found] = ((KfEffectKind)(code));
            found++;
        }
    }
    ...
    if (selection != KF_MENU_RESULT_CANCELLED) {
        if (player_state.vitals.current_mp < magic_records[((s32)(selection))].mp_cost)
            return selection;
        player_state.vitals.current_mp -= magic_records[((s32)(selection))].mp_cost;
...
void menu_option_root(void)
{
    ...
    for (;;) {
        menu_present_frame();
        if (selection != KF_EQUIP_MENU_NONE || ((s32)(phase)) == ((s32)(selection))) {
            menu_frame_begin();
            menu_draw_name_list();

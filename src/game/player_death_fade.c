...
{
    lighting_set_color_matrix(color_from, &color_matrix_table[((s32)(KF_GAME_COLOR_BLACK))], blend);
    matrix_interpolate(&color_matrix_table[((s32)(KF_GAME_COLOR_WHITE))],
        &color_matrix_table[((s32)(KF_GAME_COLOR_BLACK))], &game_graphics_runtime.render_state.effect_color_matrix, blend);
...
void player_death_update_reverse_fade(void)
{
    ...
    lighting_set_color_matrix(&color_matrix_table[((s32)(KF_GAME_COLOR_BLACK))],
        &color_matrix_table[((s32)(KF_GAME_COLOR_DEFAULT))], *blend);
    matrix_interpolate(&color_matrix_table[((s32)(KF_GAME_COLOR_BLACK))],
        &color_matrix_table[((s32)(KF_GAME_COLOR_WHITE))], &game_graphics_runtime.render_state.effect_color_matrix, *blend);
    fog_interpolate_near(0, player_death_saved_fog_near, *blend);
    game_graphics_runtime.hud_brightness = (*blend * KF_HUD_DEFAULT_BRIGHTNESS) >> KF_FIXED12_BITS;
    *blend += PLAYER_DEATH_FADE_STEP;
    if (*blend >= KF_FIXED12_ONE) {
        player_death_apply_visual_fade(&color_matrix_table[((s32)(KF_GAME_COLOR_DEFAULT))], 0);
        player_state.update_state = KF_PLAYER_UPDATE_NORMAL;
    } else {
        player_death_apply_visual_fade(
            &color_matrix_table[((s32)(KF_GAME_COLOR_DEFAULT))], KF_FIXED12_ONE - *blend);
    }
}

        ...
        if (confirm == KF_MENU_CONFIRM_REQUESTED && cursor != KF_MENU_SAVE_RETURN_ROW) {
            ...
            if (result == KF_MENU_CONFIRM_CANCELLED) {
                ...
                if (cursor < KF_SAVE_SLOT_COUNT) {
                    ...
                    status.operation = save_system_write_slot(KF_ENUM_DECODE(KfSaveSlotId, cursor + KF_ENUM_ENCODE(s16, KF_SAVE_SLOT_FIRST)));
    ...
    for (;;) {
        ...
        if (confirm == KF_MENU_CONFIRM_REQUESTED && cursor != KF_MENU_LOAD_RETURN_ROW) {
            ...
            if (result == KF_MENU_CONFIRM_CANCELLED) {
                ...
                if (save_system_read_slot(KF_ENUM_DECODE(KfSaveSlotId, cursor + KF_ENUM_ENCODE(s16, KF_SAVE_SLOT_FIRST))) != KF_SAVE_RESULT_OK) {
                    while (PadRead(1) == 0) {
                        menu_frame_begin();
                        menu_add_frame_quad();
                        menu_draw_dialog_frame(summaries, KF_ENUM_DECODE(KfSaveSlotOverlay, cursor));

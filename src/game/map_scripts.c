...
void map_ambient_script_floor3(void)
{
    if (player_state.motion_state.fields.map_cell.coords.x >= 15 && player_state.motion_state.fields.map_cell.coords.x < 18
        ...
        if (magic_records[((u8)(KF_MAGIC_RESIST_FIRE))].learned == KF_MAGIC_UNLEARNED || magic_records[((u8)(KF_MAGIC_BLESS))].learned == KF_MAGIC_UNLEARNED) {
            magic_records[((u8)(KF_MAGIC_RESIST_FIRE))].learned = KF_MAGIC_LEARNED;
            magic_records[((u8)(KF_MAGIC_BLESS))].learned = KF_MAGIC_LEARNED;
...
void map_reveal_fade(void)
{
    ...
    for (blend = 0; blend < KF_FIXED12_ONE + 1; blend += MAP_REVEAL_FADE_IN_STEP) {
        lighting_set_color_matrix(&color_matrix_table[((s32)(KF_GAME_COLOR_DEFAULT))], &color_matrix_table[((s32)(KF_GAME_COLOR_WHITE))], blend);
...
void map_action_script_floor3(void)
{
    if (item_stock[((u8)(KF_ITEM_STOCK_PLAYER))][((u8)(KF_ITEM_WIND_BLADE_BRACELET))] != 0) {
        if (magic_records[((u8)(KF_MAGIC_WIND_CUTTER))].learned == KF_MAGIC_UNLEARNED) {
            magic_records[((u8)(KF_MAGIC_WIND_CUTTER))].learned = KF_MAGIC_LEARNED;
            notify_enqueue(KF_NOTIFICATION_MAGIC_LEARNED);
        }
    }
    if ((map_event_pool[1].dialogue.word & MAP_DIALOGUE_TRIGGER_MASK)
            == MAP_DIALOGUE_STARTED(3)) {
        if (magic_records[((u8)(KF_MAGIC_FIRE_BALL))].learned == KF_MAGIC_UNLEARNED) {
            magic_records[((u8)(KF_MAGIC_FIRE_BALL))].learned = KF_MAGIC_LEARNED;
            notify_enqueue(KF_NOTIFICATION_MAGIC_LEARNED);
        }
    }
}
...
void map_event_interact(KfMapEvent *event)
{
    ...
    case KF_CHARACTER_KEY_OF_THE_DEAD_EXCHANGE:
        if (item_stock[((u8)(KF_ITEM_STOCK_PLAYER))][((u8)(KF_ITEM_GOLD_CROSS))] != 0 && map_event_pool[2].dialogue.fields.stage == 1
    ...
    case KF_CHARACTER_HEALING_EXCHANGE:
        if (item_stock[((u8)(KF_ITEM_STOCK_PLAYER))][((u8)(KF_ITEM_MIRROR_OF_TRUTH))] != 0 && map_event_pool[2].dialogue.fields.stage == 2
            && map_event_pool[2].dialogue.fields.page < 2) {
            magic_records[((u8)(KF_MAGIC_HEALING))].learned = KF_MAGIC_LEARNED;
    ...
    case KF_CHARACTER_HARP_EXCHANGE:
        if (item_stock[((u8)(KF_ITEM_STOCK_PLAYER))][((u8)(KF_ITEM_DRAGON_KING_GRASS_FRUIT))] != 0 && map_event_pool[1].dialogue.fields.stage == 2
    ...
    if (game_graphics_runtime.notification_state.control.effect_phase == KF_NOTIFICATION_IDLE
        ...
        switch (event->behavior) {
            case KF_MAP_EVENT_BEHAVIOR_SHOP:
                ...
                menu_enter_mode(KF_MENU_MODE_SHOP, ((KfItemStockBank)(((u8)(event->character_id)))));

...
void item_menu_buy(s32 shop_bank);
void item_menu_sell(s32 shop_bank);
...
void item_menu_root(s32 shop_bank)
{
...
void item_menu_buy(s32 shop_bank)
{
    ...
    inv = item_stock[((s32)(shop_bank))];
    found = 0;
    for (slot = ((s32)(KF_ITEM_VERDITE)); slot < KF_ITEM_COUNT; slot++) {
        if (inv[slot] != 0 && item_stock[((u8)(KF_ITEM_STOCK_PLAYER))][slot] < KF_ITEM_STACK_CAPACITY) {
            for (j = 0; j < MENU_GLYPHS_PER_ROW; j++)
                entries[found][j] = item_name_rows[slot].codes[j];
            available[found] = inv[slot];
            index[found] = ((KfObjectId)(slot));
            found++;
        }
    }
    ...
    for (;;) {
        ...
        if (confirm == KF_MENU_CONFIRM_REQUESTED) {
            if (menu_list_interact(&ctx, KF_MENU_CONFIRM_BUY,
                ...
                selection = ((u8)(index[ctx.selected_index]));
        }
        confirm = KF_MENU_CONFIRM_IDLE;
        ...
        load_selected_model:
            ...
            if (player_state.gold
                    < item_buy_prices[((u8)(index[ctx.selected_index]))][((s32)(shop_bank)) - ((s32)(KF_ITEM_STOCK_FIRST_SHOP))]) {
                menu_play_input_sound(MENU_SOUND_CANCEL_OR_ERROR);
            } else {
                menu_play_input_sound(MENU_SOUND_CONFIRM);
                confirm = KF_MENU_CONFIRM_REQUESTED;
...
void item_menu_sell(s32 shop_bank)
{
    ...
    for (;;) {
        menu_present_frame();
        if (confirm == KF_MENU_CONFIRM_REQUESTED) {
            selection = ((s32)(menu_list_interact(&ctx, KF_MENU_CONFIRM_SELL,

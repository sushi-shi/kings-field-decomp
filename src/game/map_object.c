...
enum {
    ...
    MAP_SWING_DOOR_YAW_STEP = KF_ANGLE_QUARTER_TURN / ((u16)(KF_MAP_OBJECT_SWING_OPEN_END)),
    ...
    MAP_REVEAL_LIFT = KF_MAP_OBJECT_REVEAL_DEPTH
        + (((u16)(KF_MAP_OBJECT_REVEAL_SETTLE_END)) - 1) * KF_MAP_OBJECT_REVEAL_SETTLE_STEP,
...
s32 map_object_pool_find_interaction_from(s32 start_index, s32 x, s32 z, s32 extra_radius)
{
    ...
    for (; index < KF_MAP_OBJECT_CAPACITY; index++, object++) {
        ...
        definition = &map_object_state.definitions.entries[((u8)(object->object_id))];
...
void map_object_pool_trigger_link(u8 link_id)
{
    ...
    do {
        ...
        case KF_MAP_OBJECT_OP_RELEASE_LONG_SWING:
            ...
            break;
        default:
            if (map_object_state.definitions.entries[((u8)(object->object_id))]
                        .behavior_type
                ...
                map_object_start_action_if_idle(
                    object, map_object_state.definitions.entries[((u8)(object->object_id))].behavior_type);
            }
            break;
        }
        object++;
...
void map_object_pool_clear_link(u8 link_id)
{
    ...
    do {
        if ((definitions[((u8)(object->object_id))].behavior_type < KF_MAP_OBJECT_OP_LINK_CLEAR_LAST
             || definitions[((u8)(object->object_id))].behavior_type == KF_MAP_OBJECT_OP_LINK_CLEAR_LAST)
            && object->link.fields.link_id == link_id) {
            object->link.fields.link_id = KF_MAP_LINK_NONE;
        }
        object++;
...
void map_object_pool_update(void)
{
    ...
    for (count = KF_MAP_OBJECT_CAPACITY; count != 0; object++, count--) {
                ...
                if (timer == KF_MAP_OBJECT_PROGRESS_INIT) {
                    if (object->object_id == KF_MAP_OBJECT_HINGED_DOOR) {
                        audio_play_spatial_default_range(
                            &gameplay_sound_refs[KF_GAMEPLAY_SOUND_HINGED_DOOR],
                            &object->position,
                            KF_AUDIO_MAX_VOLUME);
                    } else {
                        audio_play_spatial_default_range(
                            &gameplay_sound_refs[KF_GAMEPLAY_SOUND_STONE_PASSAGE],
                            &object->position,
                            KF_AUDIO_MAX_VOLUME);
                    }
                }
                if (timer == KF_MAP_OBJECT_SWING_OPEN_LAST) {
                    map_object_mark_collision_edge(object,
                        KF_MAP_CELL_FLOOR,
                        object->rotation.angles.y - KF_ANGLE_QUARTER_TURN);
        ...
        case KF_MAP_OBJECT_OP_PROJECTILE_EMITTER:
            ...
            if (object->action_timer == KF_MAP_OBJECT_PROGRESS_INIT) {
                if (player_distance_to_point(object->position.vx,
            ...
            case KF_MAP_OBJECT_PROJECTILE_EMITTER:
                ...
                object->action_timer = ((KfMapObjectProgress)((rand() >> MAP_EMITTER_COUNTDOWN_RANDOM_SHIFT) + MAP_EMITTER_COUNTDOWN_BASE));
                break;
            ...
            case KF_MAP_OBJECT_WIND_CUTTER_EMITTER:
                ...
                effect_pool_construct(
                    ...
                    &point,
                    &direction,
                    (&object->rotation.vector), (KF_EFFECT_SOUND_PLAY));
                object->action_timer = ((KfMapObjectProgress)((rand() >> MAP_EMITTER_COUNTDOWN_RANDOM_SHIFT) + MAP_EMITTER_COUNTDOWN_BASE));
                break;
            case KF_MAP_OBJECT_BOSS_PROJECTILE_EMITTER:
                ...
                effect_pool_construct(object->link.fields.spawn.effect_id,
                    ...
                    &point,
                    &direction,
                    (&object->rotation.vector), (effect_sound_request(rand() < MAP_BOSS_EMITTER_SOUND_RANDOM_LIMIT)));
                object->action_timer = ((KfMapObjectProgress)((rand() >> MAP_EMITTER_COUNTDOWN_RANDOM_SHIFT)
        ...
        case KF_MAP_OBJECT_OP_RELEASE_LONG_SWING:
            if (object->link.fields.link_id == KF_MAP_LINK_NONE
                && object->action_timer == KF_MAP_OBJECT_PROGRESS_INIT) {
                effect_state.records[object->link.fields.action_parameter.effect_index].phase
                    = KF_EFFECT_HAZARD_RELEASE_REQUEST;
                object->action_timer = KF_MAP_OBJECT_PROGRESS_RUNNING;
            }
            break;
        case KF_MAP_OBJECT_OP_EFFECT_SWITCH:
            ...
            if (object->link.fields.link_id == KF_MAP_LINK_NONE) {
                effect_state.records[object->link.fields.action_parameter.effect_index]
                    .visual.animation_phase
                    = (KF_FIXED12_ONE - 1);
        ...
        case KF_MAP_OBJECT_OP_REVEAL_MAP_PIECE:
            ...
            if (object->action_timer == KF_MAP_OBJECT_PROGRESS_INIT) {
                ...
                if (player_state.progress_state.current_floor == KF_FLOOR_3) {
                    ...
                    if (map_runtime_state.world_state.floors[0].script.floor1.revival_enabled == KF_MAP_SCRIPT_UNSET) {
                        audio_play_spatial_default_range(
                            &gameplay_sound_refs[KF_GAMEPLAY_SOUND_FLOOR1_REVIVAL],
                            &object->position,
                            KF_AUDIO_MAX_VOLUME);
                        map_runtime_state.world_state.floors[0].script.floor1.revival_enabled = KF_MAP_SCRIPT_SET;
                    }
                }
                object->action_timer++;
            }
        ...
        case KF_MAP_OBJECT_OP_RESTORE_POINT:
            if (object->link.fields.link_id == KF_MAP_LINK_NONE) {
                ...
                object->rotation.angles.y
                    = (object->rotation.angles.y + MAP_RESTORE_POINT_YAW_STEP) & KF_ANGLE_WRAP_MASK;
            }
            break;
        }
    }

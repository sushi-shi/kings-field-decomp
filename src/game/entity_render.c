...
void render_actor_sprite(KfEffectRecord *sprite)
{
    ...
    if (sprite->animation_clip == KF_ANIMATION_CLIP_NONE) {
        ...
        render_enqueue_sprite(&effect_billboard_sprites[((u8)(sprite->render_id.billboard))], 0, KF_SPRITE_DEPTH_CUE_NORMAL);
    } else {
        MulMatrix2(&game_graphics_runtime.render_state.view_matrix, &model);
        SetRotMatrix(&model);
        SetTransMatrix(&model);
        asset = ((u8)(sprite->render_id.model)) + KF_ASSET_EFFECT_FIRST;

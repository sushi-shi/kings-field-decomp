...
void menu_drop_item(void)
{
    ...
    for (;;) {
        ...
        if (confirm == KF_MENU_CONFIRM_REQUESTED) {
            selection = ((s32)(menu_list_interact(&ctx, KF_MENU_CONFIRM_DROP,
...
KfMenuResult menu_save_load_hub(void)
{
    ...
    s32 cursor = ((s32)(KF_MENU_SYSTEM_ACTION_LOAD));
    KfMenuConfirmState confirm = KF_MENU_CONFIRM_IDLE;
    s32 input = 0;
    s32 prev;
    ...
    for (;;) {
        ...
        if (PAD_PRESSED(input, prev, PADLup)) {
            ...
            if (cursor != KF_MENU_SYSTEM_RETURN_ROW)
                ...
                cursor = ((s32)(KF_MENU_SYSTEM_ACTION_LOAD));
        } else if (PAD_PRESSED(input, prev, PADRright)) {
            menu_play_input_sound(MENU_SOUND_CONFIRM);
            confirm = KF_MENU_CONFIRM_REQUESTED;
            if (cursor == KF_MENU_SYSTEM_RETURN_ROW) {
                ...
                action = ((KfMenuSystemAction)(cursor));
            }
        } else if (PAD_PRESSED(input, prev, PADRdown)) {
            menu_play_input_sound(MENU_SOUND_CANCEL_OR_ERROR);
            result = KF_MENU_RESULT_CANCELLED;
...
KfMenuResult menu_save_panel(void)
{
    ...
    status = ((s32)(save_system_read_catalog(summaries)));
    if (status != ((s32)(KF_SAVE_RESULT_OK)) && status != ((s32)(KF_SAVE_RESULT_NO_SPACE))) {
        menu_play_input_sound(MENU_SOUND_CURSOR);
        while (PadRead(1) == 0) {
            menu_frame_begin();
            menu_add_frame_quad();
            menu_draw_dialog_frame(summaries, ((KfSaveSlotOverlay)(cursor)));
    ...
    for (;;) {
        ...
        if (confirm == KF_MENU_CONFIRM_REQUESTED && cursor != KF_MENU_SAVE_RETURN_ROW) {
            if (cursor == KF_MENU_SAVE_FORMAT_ROW) {
                status = ((s32)(save_file_cleanup_temporary()));
...
void menu_config_panel(void)
{
    ...
    do {
        ...
        if (PAD_PRESSED(pad, prev, PADLup)) {
            ...
            if (row != KF_MENU_CONFIG_RETURN_ROW) {
                menu_play_input_sound(MENU_SOUND_CONFIRM);
                states[row] = ((KfPlayerOption)(states[row] == KF_PLAYER_OPTION_OFF));
            }
...
void menu_draw_stats_header(void)
{
    ...
    gs.position.x = 0xfb;
    gs.position.y += row_step;
    menu_format_number(((u8)(player_state.progress_state.current_floor)), MENU_STATS_VALUE_DIGITS, KF_FORMAT_PAD_SPACES, gs.glyphs.codes);
    menu_draw_number(&menu_assets.number_atlas, &gs);
...
void menu_draw_name_list(void)
{
    ...
    if (player_state.equipped_weapon_id != KF_OBJECT_NONE) {
        gs.glyphs =
            item_name_rows[((u8)(player_state.equipped_weapon_id))];
        menu_draw_string(&menu_assets.glyph_atlas, &gs);
    }
    gs.position.y += EQUIPMENT_NAME_ROW_STEP;
    if (player_state.selected_magic_id != KF_MAGIC_NONE) {
        gs.glyphs =
            magic_name_rows[((u8)(player_state.selected_magic_id))];
        menu_draw_string(&menu_assets.glyph_atlas, &gs);
    }
    gs.position.y += EQUIPMENT_NAME_ROW_STEP;
    if (player_state.equipped_shield_id != KF_OBJECT_NONE) {
        gs.glyphs =
            item_name_rows[((u8)(player_state.equipped_shield_id))];
        menu_draw_string(&menu_assets.glyph_atlas, &gs);
    }
    gs.position.y += EQUIPMENT_NAME_ROW_STEP;
    if (player_state.equipped_head_armor_id != KF_OBJECT_NONE) {
        gs.glyphs =
            item_name_rows[((u8)(player_state.equipped_head_armor_id))];
        menu_draw_string(&menu_assets.glyph_atlas, &gs);
    }
    gs.position.y += EQUIPMENT_NAME_ROW_STEP;
    if (player_state.equipped_body_armor_id != KF_OBJECT_NONE) {
        gs.glyphs =
            item_name_rows[((u8)(player_state.equipped_body_armor_id))];
        menu_draw_string(&menu_assets.glyph_atlas, &gs);
    }
    gs.position.y += EQUIPMENT_NAME_ROW_STEP;
    if (player_state.equipped_arm_armor_id != KF_OBJECT_NONE) {
        gs.glyphs =
            item_name_rows[((u8)(player_state.equipped_arm_armor_id))];
        menu_draw_string(&menu_assets.glyph_atlas, &gs);
    }
    gs.position.y += EQUIPMENT_NAME_ROW_STEP;
    if (player_state.equipped_leg_armor_id != KF_OBJECT_NONE) {
        gs.glyphs =
            item_name_rows[((u8)(player_state.equipped_leg_armor_id))];
        menu_draw_string(&menu_assets.glyph_atlas, &gs);
    }
    gs.position.y += EQUIPMENT_NAME_ROW_STEP;
    if (player_state.equipped_accessory_id != KF_OBJECT_NONE) {
        gs.glyphs =
            item_name_rows[((u8)(player_state.equipped_accessory_id))];
        menu_draw_string(&menu_assets.glyph_atlas, &gs);
    }
}
...
void menu_item_model_preview(s32 item_id)
{
    ...
    if (item_id != KF_OBJECT_NONE) {
        ...
        menu_format_number(item_stock[((u8)(KF_ITEM_STOCK_PLAYER))][((s32)(item_id))], MENU_ITEM_PREVIEW_QUANTITY_DIGITS, KF_FORMAT_PAD_SPACES, gs.glyphs.codes);
        menu_draw_number(&menu_assets.number_atlas, &gs);
    }
}
...
void menu_draw_item_detail(s32 item_id, s32 shop_bank, KfTradeMode price_mode)
{
    MenuGlyphString gs;
    MATRIX rot;
    MATRIX lsrc;
    ...
    if (price_mode == KF_TRADE_BUY) {
        price = item_buy_prices[((s32)(item_id))]
            [((s32)(shop_bank)) - ((s32)(KF_ITEM_STOCK_FIRST_SHOP))];
    } else {
        price = item_sell_prices[((s32)(item_id))]
            [((s32)(shop_bank)) - ((s32)(KF_ITEM_STOCK_FIRST_SHOP))];
    }
    ...
    menu_blit_sprite_translucent(
        ...
        &menu_window_layouts[((s32)(KF_MENU_WINDOW_SHOP))].rows[KF_SHOP_ROW_GOLD].position);
    menu_draw_string(&menu_assets.glyph_atlas, &menu_window_layouts[((s32)(KF_MENU_WINDOW_SHOP))].rows[KF_SHOP_ROW_GOLD]);

    gs.position.x = menu_window_layouts[((s32)(KF_MENU_WINDOW_SHOP))].rows[KF_SHOP_ROW_GOLD].position.x + MENU_ITEM_DETAIL_GOLD_X_OFFSET;
    gs.position.y = menu_window_layouts[((s32)(KF_MENU_WINDOW_SHOP))].rows[KF_SHOP_ROW_GOLD].position.y;
...
void menu_add_marker_quad(void)
{
    AddPrim((void *)(game_graphics_runtime.display_state.ordering_table + MENU_MARKER_OT_DEPTH),
            (void *)(&menu_assets.mid_depth_quads[((u8)(game_graphics_runtime.display_state.buffer_index))]));
}

void menu_add_frame_quad(void)
{
    AddPrim((void *)game_graphics_runtime.display_state.ordering_table,
            (void *)(&menu_assets.foreground_quads[((u8)(game_graphics_runtime.display_state.buffer_index))]));
}
...
void menu_draw_dialog_frame(const KfSaveSlotSummary *summaries, KfSaveSlotOverlay slot_overlay)
{
    ...
    for (i = 0; i < KF_SAVE_SLOT_COUNT; i++) {
        ...
        if ((s32)summaries[i].current_hp > 0) {
            ...
            gs.position.x = 286;
            menu_format_number(((u32)(summaries[i].current_floor)), 1, KF_FORMAT_PAD_SPACES, gs.glyphs.codes);
            menu_draw_number(&menu_assets.number_atlas, &gs);
    ...
    s32 preview_id, u32 shop_bank, KfTradeMode price_mode)
{
    ...
    if (preview_mode == KF_MENU_PREVIEW_ITEM_MODEL) {
        menu_item_model_preview(((s32)(preview_id)));
    } else if (preview_mode == KF_MENU_PREVIEW_ITEM_DETAIL) {
        menu_draw_item_detail(((s32)(preview_id)), shop_bank, price_mode);
    } else if (preview_mode == KF_MENU_PREVIEW_MAGIC_ICON
            && preview_id != ((s32)(KF_MAGIC_NONE))) {
    ...
    const KfSaveSlotSummary *summaries)
{
    ...
    if (window_kind == KF_MENU_WINDOW_SAVE || window_kind == KF_MENU_WINDOW_LOAD)
        overlay = ((KfSaveSlotOverlay)(highlight_row));
...
void menu_frame_begin(void)
{
    ...
    game_graphics_runtime.display_state.primitive_buffer =
        &game_graphics_runtime.display_state.primitive_buffers[((u8)(game_graphics_runtime.display_state.buffer_index))];
    game_graphics_runtime.display_state.ordering_table =
        game_graphics_runtime.display_state.ordering_tables[((u8)(game_graphics_runtime.display_state.buffer_index))].entries;
...
void menu_present_frame(void)
{
    DrawSync(0);
    VSync(0);
    PutDrawEnv(&game_graphics_runtime.display_draw_environments[((u8)(game_graphics_runtime.display_state.buffer_index))]);
    PutDispEnv(&game_graphics_runtime.display_disp_environments[((u8)(game_graphics_runtime.display_state.buffer_index))]);
...
void menu_list_init(KfMenuList *list, KfMenuWindowKind window_kind, s32 row)
{
    ...
    for (i = 0; i < MENU_GLYPHS_PER_ROW; i++) {
        list->title.glyphs.codes[i] = menu_window_layouts[((s32)(window_kind))].rows[row].glyphs.codes[i];
    }
    list->list_x = 0x16;
    list->list_y = 0x26;
    list->entry_count = 0;
...
void menu_format_number(s32 value, s32 count, s32 padding_mode, s16 *out)
{
    s32 i = 0;
    s32 blank;
...
u32 menu_load_item_model(s32 item_id)
{
    u8 *asset;

    menu_release_item_model();
    if (item_id != KF_OBJECT_NONE) {
        if (cd_file_load_table_entry(&asset, ((s32)(item_id))) != KF_RESOURCE_LOADED) {
...
u32 menu_load_item_texture(KfMenuTextureId texture_id)
{

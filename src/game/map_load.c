...
void map_restore_floor_state(void)
{
    ...
    if (*in++ == 1) {
        ...
        for (i = 0; i < KF_MAP_EVENT_CAPACITY; i++, event++) {
            event->state = ((KfMapEventState)(*in++));
        ...
        for (i = 0; i < KF_MAP_OBJECT_CAPACITY; i++, object++) {
            object->object_id = ((KfObjectId)(*in++));
        }
    ...
    case KF_FLOOR_5:
        ...
        if (item_stock[((u8)(KF_ITEM_STOCK_PLAYER))][((u8)(KF_ITEM_DRAGON_SWORD))] != 0 || item_stock[((u8)(KF_ITEM_STOCK_PLAYER))][((u8)(KF_ITEM_MOONLIGHT_SWORD))] != 0
...
void map_load_floor(void)
{
    ...
    game_graphics_runtime.render_state.effect_color_matrix = color_matrix_table[((s32)(KF_GAME_COLOR_WHITE))];
}

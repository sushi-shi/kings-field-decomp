...
s32 memory_card_show_status_message(KfSaveStatus status);
...
KfSaveResult save_system_read_catalog(KfSaveSlotSummary *summaries)
{
    ...
    if (result == KF_SAVE_RESULT_OK) {
        ...
        for (index = 0; index < KF_SAVE_DIRECTORY_ENTRIES; index++) {
            KfEnumStorage<KfSaveSlotId, u8> slot = header->directory.slot_ids[index];
...
s32 memory_card_show_status_message(KfSaveStatus status)
{
    KfEnumStorage<KfSaveStatus, s16> status_value = status;
...
void talk_show_dialogue_page(KfFloorId floor,
    u8 stage,

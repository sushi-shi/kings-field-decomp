...
enum {
    ...
    EFFECT_MOONLIGHT_INITIAL_CONTROL_BYTE = 0xff,
    ...
    const SVECTOR *direction, ...)
{
    ...
    if (record != NULL) {
        ...
        magic = &effect_state.magic.entries[((u8)(record->kind))];
        ...
        case KF_MAGIC_WIND_CUTTER:
            ...
            if (((KfEffectSoundRequest)(va[2])) != KF_EFFECT_SOUND_SILENT) {
                audio_play_spatial_default_range(&magic->sounds[0],
                                                 &record->position, KF_AUDIO_MAX_VOLUME);
            }
            break;
        ...
        initialize_lightning_bolt:
            ...
            if (((KfEffectSoundRequest)(va[2])) != KF_EFFECT_SOUND_SILENT) {
                audio_play_spatial_range(
                    &effect_state.magic.entries[((u8)(KF_MAGIC_LIGHTNING_BOLT))].sounds[0],
                    &record->position, KF_AUDIO_MAX_VOLUME,
        ...
        initialize_lightning_impact:
            ...
            setVector(&record->rotation.vector, 0, 0, 0);
            audio_play_spatial_range(
                &effect_state.magic.entries[((u8)(KF_MAGIC_LIGHTNING_BOLT))].sounds[1],
                &record->position, KF_AUDIO_MAX_VOLUME,
                KF_AUDIO_EXTENDED_MAX_DISTANCE,
                KF_AUDIO_EXTENDED_ATTENUATION_DISTANCE);
            break;
        ...
        case KF_MAGIC_FIRE_WALL:
            ...
            record->scale_y = 0;
            {
                KfEffectGroundBranchRole branch_role = ((KfEffectGroundBranchRole)(*(u16 *)(va + 1)));
        ...
        case KF_EFFECT_KIND_MOONLIGHT_PROJECTILE:
            ...
            record->rotation.vector.vx = -record->rotation.vector.vx;
            audio_play_spatial_range(
                &effect_state.magic.entries[((u8)(KF_EFFECT_KIND_RADIAL_BLAST))].sounds[1],
                &record->position, KF_AUDIO_MAX_VOLUME,
                KF_AUDIO_EXTENDED_MAX_DISTANCE,
                KF_AUDIO_EXTENDED_ATTENUATION_DISTANCE);
            break;
        ...
        case KF_EFFECT_KIND_RADIAL_BLAST_ALTERNATE:
            ...
            record->animation_clip = KF_ANIMATION_CLIP_FIRST;
            if (((KfEffectSoundRequest)(va[1])) != KF_EFFECT_SOUND_SILENT) {
                audio_play_spatial_range(
                    &effect_state.magic.entries[((u8)(KF_EFFECT_KIND_RADIAL_BLAST))].sounds[0],
                    &record->position, KF_AUDIO_MAX_VOLUME,
                    KF_AUDIO_EXTENDED_MAX_DISTANCE,
                    KF_AUDIO_EXTENDED_ATTENUATION_DISTANCE);
            }
        ...
        case KF_EFFECT_KIND_HOMING_PROJECTILE_ALTERNATE:
            ...
            record->rotation.vector = *(const SVECTOR *)va[1];
            record->rotation.vector.vx = -record->rotation.vector.vx;
            record->control.target_mode = ((KfEffectHomingMode)(*(u8 *)(va + 2)));
            record->direction.vector = record->rotation.vector;
            record->scale_y = KF_FIXED12_ONE / 2;
            record->scale_x = KF_FIXED12_ONE / 2;
            if (((KfEffectSoundRequest)(va[3])) != KF_EFFECT_SOUND_SILENT) {
                audio_play_spatial_default_range(
                    &effect_state.magic.entries[((u8)(KF_EFFECT_KIND_HOMING_PROJECTILE))].sounds[0],
                    &record->position, KF_AUDIO_MAX_VOLUME);
            }
            break;
...
void effect_pool_set_current(KfEffectRecord *effect)
{
    ...
    effect_state.current_magic = &effect_state.magic.entries[((u8)(effect->kind))];
}

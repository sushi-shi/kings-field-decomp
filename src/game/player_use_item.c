...
void actor_show_info_image(const KfActor *actor)
{
    ...
    render_frame(NULL, NULL);
    enemy_info_image_path_template[3] = '0' + ((u8)(player_state.progress_state.current_floor));
...
void map_event_show_person_image(const KfMapEvent *event)
{
    render_frame(NULL, NULL);
    render_frame(NULL, NULL);
    person_image_path_template[8] = '0' + ((u8)(event->character_id)) / 10;
    person_image_path_template[9] = '0' + ((u8)(event->character_id)) % 10;
...
void player_use_item(KfObjectId item_id)
{
        ...
        for (;;) {
            ...
            case KF_MAP_OBJECT_TALL_HINGED_DOOR_PARTNER:
                if (object->link.fields.link_id == KF_MAP_LINK_NONE) {
                    ...
                    used = KF_TRUE;
                    if (object->link.fields.link_id == ((u8)(item_id))) {
    ...
    case KF_ITEM_WIND_SEAL_STONE:
        for (;;) {
            ...
            object = &map_object_state.objects[index];
            if (object->object_id == ((KfObjectId)(((u8)(item_id))))) {
                if (object->link.fields.link_id == KF_MAP_LINK_NONE) {
                    notify_enqueue(KF_NOTIFICATION_NOTHING_HAPPENS);
                } else {
                    item_stock[((u8)(KF_ITEM_STOCK_PLAYER))][((u8)(object->object_id))] = 0;
                    used = KF_TRUE;
    ...
    case KF_ITEM_ILLUSION_STAFF:
        ...
        if (item_stock[((u8)(KF_ITEM_STOCK_PLAYER))][((u8)(KF_ITEM_ILLUSION_STAFF))] != 0) {
            item_stock[((u8)(KF_ITEM_STOCK_PLAYER))][((u8)(KF_ITEM_ILLUSION_STAFF))]--;
        }
        return;

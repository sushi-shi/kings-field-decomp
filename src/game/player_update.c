        ...
        if (player_state.weapon_magic_delay == PLAYER_WEAPON_MAGIC_READY) {
            if (player_state.weapon_magic_shots_remaining != 0) {
                ...
                case KF_ITEM_TRIPLE_FANG:
                    ...
                    record = &effect_state.magic.entries[((u8)(KF_EFFECT_KIND_HOMING_PROJECTILE))];
                    player_state.weapon_magic_delay = PLAYER_TRIPLE_FANG_MAGIC_DELAY;
                    break;
                case KF_ITEM_FLAME_SWORD:
                    if (effect_state.magic.entries[((u8)(KF_MAGIC_FIRE_BALL))].learned
                        == KF_MAGIC_UNLEARNED) {
                        goto cancel;
                    }
                    effect = KF_MAGIC_FIRE_BALL;
                    record = &effect_state.magic.entries[((u8)(KF_MAGIC_FIRE_BALL))];
                ...
                case KF_ITEM_MOONLIGHT_SWORD:
                    ...
                    record = &effect_state.magic.entries[((u8)(KF_EFFECT_KIND_RADIAL_BLAST))];
                ...
                case KF_ITEM_COLICHEMARDE:
                    ...
                    record = &effect_state.magic.entries[((u8)(KF_MAGIC_LIGHT_NEEDLE))];
                    player_state.weapon_magic_delay = PLAYER_WEAPON_MAGIC_READY;
                    break;
                default:
                cancel:
    ...
    if (player_state.update_state != KF_PLAYER_UPDATE_NORMAL
        ...
        if (player_state.update_state >= KF_PLAYER_DAMAGE_FRAME_END) {
            ...
            player_state.view_rotation_offset
                = player_damage_camera_offsets[((u8)(KF_PLAYER_UPDATE_NORMAL))];
            if (player_state.vitals.current_hp == 0) {
                player_death_begin();
            }
        } else {
            player_state.view_rotation_offset
                = player_damage_camera_offsets[((u8)(player_state.update_state))];

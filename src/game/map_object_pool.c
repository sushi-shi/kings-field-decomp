...
void map_apply_copy_region(KfMapCopyRegionId region_id)
{
    ...
    region = &map_copy_regions[((u8)(region_id))];
...
{
    const KfMapObjectDefinition *definition = &map_object_state.definitions.entries[((u8)(object->object_id))];
...
void map_object_pool_load(const KfMapObjectPlacement *placements)
{
    ...
    for (;;) {
        ...
        fill:
            object->object_id = KF_OBJECT_NONE;
        } else if (((KfObjectId)(placement->object_id)) != KF_OBJECT_NONE) {
            object_id = ((KfObjectId)(placement->object_id));
...
s32 map_object_pool_find_near_point(s32 point_x, s32 point_z, s32 radius_padding)
{
    ...
    for (index = 0; index < KF_MAP_OBJECT_CAPACITY; index++, object++) {
        ...
        radius = map_object_state.definitions.entries[((u8)(object->object_id))].collision_radius;
        if (radius == 0) {
            continue;
        }

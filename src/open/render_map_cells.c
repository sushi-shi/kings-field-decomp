...
void render_map_cell(s32 col, s32 row, KfCellVisibility visibility)
{
    ...
    orientation = ((u8)(map_cell_orientation_grid.cells[row][col])) - 1;
    ...
    if (orientation == ((u8)(KF_MAP_ORIENT_QUARTER_TURN)) - 1) {
        position.vz += KF_MAP_TILE_SIZE;
    } else if (orientation == ((u8)(KF_MAP_ORIENT_HALF_TURN)) - 1) {
        position.vx += KF_MAP_TILE_SIZE;
        position.vz += KF_MAP_TILE_SIZE;
    } else if (orientation == ((u8)(KF_MAP_ORIENT_THREE_QUARTER_TURN)) - 1) {
        position.vx += KF_MAP_TILE_SIZE;
    }

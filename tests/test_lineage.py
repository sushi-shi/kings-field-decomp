...
class OverlayLineageTests(unittest.TestCase):
    ...
    def test_evidence_matches_contiguous_function_inventory(self) -> None:
        ...
        self.assertEqual({row.address_delta for row in rows}, {0x2022C})
        self.assertEqual(sum(row.total_words for row in rows), 1509)
        self.assertEqual(sum(row.shape_words for row in rows), 1509)

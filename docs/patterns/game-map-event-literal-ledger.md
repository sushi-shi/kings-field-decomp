...
| --- | ---: | --- | --- | --- |
| `map_event_refresh_dialogue_stage` | 26 | `0` | `event->dialogue_page_delay = 0;` | No pending page advance; ordinary zero countdown, reset on initialization or stage change. |
| `map_event_advance_animation_blocking` | 40 | `0` | `render_frame(0, 0);` | Null position/rotation overrides select the live player camera. |
| `map_event_advance_animation_blocking` | 40 | `0` | `render_frame(0, 0);` | Null position/rotation overrides select the live player camera. |
| `map_event_advance_animation_blocking` | 44 | `0` | `render_frame(0, 0);` | Null position/rotation overrides select the live player camera. |
| `map_event_advance_animation_blocking` | 44 | `0` | `render_frame(0, 0);` | Null position/rotation overrides select the live player camera. |
| `map_event_pool_load` | 50 | `0` | `u8 exhausted = 0;` | Definition-list exhaustion starts false. |
| `map_event_pool_load` | 52 | `1` | `u16 count = KF_MAP_EVENT_CAPACITY - 1;` | Inclusive countdown starts at capacity minus one; eight slots are visited. |
| `map_event_pool_load` | 55 | `1` | `if (exhausted == 1) {` | Boolean list-exhausted state after the free/end marker. |
...
| `map_event_distance_to_point` | 118 | `1` | `return -1;` | Conventional negative query-miss result, distinct from valid nonnegative distance or pool index. |
| `map_event_pool_find_target_in_cone` | 129 | `0` | `KfMapEvent *found = 0;` | Null result until an eligible event is found. |
| `map_event_pool_find_target_in_cone` | 131 | `0` | `s32 found_distance = 0;` | Distance output remains zero when no event is found; the pointer result distinguishes failure. |
| `map_event_pool_find_target_in_cone` | 133 | `1` | `u16 count = KF_MAP_EVENT_CAPACITY - 1;` | Inclusive countdown starts at capacity minus one; eight slots are visited. |
| `map_event_pool_find_target_in_cone` | 143 | `1` | `if (distance == -1) {` | Recognize the distance helper's query-miss result. |
| `map_event_pool_find_target_in_cone` | 150 | `1` | `if (angle >= KF_ANGLE_HALF_TURN + 1) {` | Exclusive half-turn boundary; the unit adjustment preserves the exact predicate. |
| `map_event_pool_find_target_in_cone` | 161 | `0` | `} while (event++, count-- != 0);` | Post-decrement zero endpoint visits the final slot before ending the fixed-size walk. |
| `map_event_pool_find_overlap` | 170 | `0` | `s16 index = 0;` | Start the pool walk at its first zero-based index. |
| `map_event_pool_find_overlap` | 175 | `1` | `event, point_x, point_z, event->radius + radius_padding) != -1) {` | A nonnegative distance means this active event overlaps the requested radius. |
| `map_event_pool_find_overlap` | 181 | `1` | `return -1;` | Conventional negative query-miss result, distinct from valid nonnegative distance or pool index. |
...
| `map_event_update_animation_loop` | 104 | `0` | `(const VECTOR *)&map_event_pool[0].reference_x,` | Use that first event slot's world position for sound playback. |
| `map_event_pool_update` | 113 | `1` | `u16 index = KF_MAP_EVENT_CAPACITY - 1;` | Inclusive countdown starts at capacity minus one; eight slots are visited. |
| `map_event_pool_update` | 134 | `0` | `if (map_dialogue_advance_gate == 0 && event->dialogue_page_delay != 0) {` | Zero gate is the dialogue tick; nonzero delay means an advance is pending. |
| `map_event_pool_update` | 134 | `0` | `if (map_dialogue_advance_gate == 0 && event->dialogue_page_delay != 0) {` | Zero gate is the dialogue tick; nonzero delay means an advance is pending. |
| `map_event_pool_update` | 136 | `0` | `if (event->dialogue_page_delay == 0) {` | Countdown expired: advance and cap the page now. |
| `map_event_pool_update` | 137 | `1` | `s32 limit = event->dialogue_pages.last_page[event->dialogue_stage - 1];` | Convert a one-based dialogue stage to its zero-based last-page array index. |
| `map_event_pool_update` | 147 | `0` | `} while (index-- != 0);` | Post-decrement zero endpoint visits the final slot before ending the fixed-size walk. |
...
53 retained occurrences.

| Function | Line | Token | Expression | Reason |
| --- | ---: | --- | --- | --- |
| `map_restore_floor_state` | 47 | `1` | `if (*in++ == 1) {` | Serialized records-present marker; only this exact byte value enables restoration. |
| `map_restore_floor_state` | 49 | `0` | `for (i = 0; i < KF_MAP_EVENT_CAPACITY; i++, event++) {` | Zero-based traversal of the explicitly named pool/group capacity. |
| `map_restore_floor_state` | 54 | `1` | `event->dialogue_pages.last_page[event->dialogue_stage - 1] = *in++;` | Convert a one-based dialogue stage to its zero-based last-page array index. |
| `map_restore_floor_state` | 60 | `1` | `if (--i != -1) {` | Negative countdown endpoint preserves zero-count skipping and the complete byte/record traversal. |
| `map_restore_floor_state` | 66 | `1` | `} while (--i != -1);` | Negative countdown endpoint preserves zero-count skipping and the complete byte/record traversal. |
| `map_restore_floor_state` | 69 | `0` | `object = &map_object_state.objects[0];` | Begin at the first record of the owning array. |
| `map_restore_floor_state` | 70 | `0` | `for (i = 0; i < KF_MAP_OBJECT_CAPACITY; i++, object++) {` | Zero-based traversal of the explicitly named pool/group capacity. |
| `map_restore_floor_state` | 75 | `1` | `while (--i != -1) {` | Negative countdown endpoint preserves zero-count skipping and the complete byte/record traversal. |
| `map_restore_floor_state` | 82 | `1` | `k = sizeof(object->link) - 1;` | Initialize the signed byte-copy countdown to the complete member extent minus one. |
| `map_restore_floor_state` | 85 | `1` | `} while (--k != -1);` | Negative countdown endpoint preserves zero-count skipping and the complete byte/record traversal. |
| `map_restore_floor_state` | 89 | `0` | `for (i = 0; i < KF_MAP_OBJECT_EFFECT_GROUP_CAPACITY; i++, object++) {` | Zero-based traversal of the explicitly named pool/group capacity. |
| `map_restore_floor_state` | 98 | `0` | `object->rotation.z = 0;` | Angular origin initializes the indicated rotation lane; no separate state encoding. |
| `map_restore_floor_state` | 99 | `0` | `object->rotation.y = 0;` | Angular origin initializes the indicated rotation lane; no separate state encoding. |
| `map_restore_floor_state` | 100 | `0` | `object->rotation.x = 0;` | Angular origin initializes the indicated rotation lane; no separate state encoding. |
| `map_restore_floor_state` | 102 | `8` | `*(u16 *)&object->link \|= *in++ << 8;` | Restore that high byte while preserving the previously read low byte. |
| `map_restore_floor_state` | 103 | `0` | `object->link.spawn_sequence = 0;` | Initialize the transient drop sequence to zero. |
| `map_restore_floor_state` | 104 | `0` | `object->link.vertical_velocity = 0;` | Start the transient drop with zero vertical velocity. |
| `map_restore_floor_state` | 108 | `0` | `for (i = 0; i < 2 * KF_MAP_OBJECT_EFFECT_GROUP_CAPACITY; i++, object++) {` | Zero-based record traversal; multiplier 2 spans the adjacent definition-drop and placement-drop groups. |
| `map_restore_floor_state` | 108 | `2` | `for (i = 0; i < 2 * KF_MAP_OBJECT_EFFECT_GROUP_CAPACITY; i++, object++) {` | Zero-based record traversal; multiplier 2 spans the adjacent definition-drop and placement-drop groups. |
| `map_restore_floor_state` | 120 | `0` | `object->rotation.x = 0;` | Angular origin initializes the indicated rotation lane; no separate state encoding. |
| `map_restore_floor_state` | 122 | `0` | `object->rotation.z = 0;` | Angular origin initializes the indicated rotation lane; no separate state encoding. |
...
| `map_restore_floor_state` | 142 | `0x33` | `map_object_pool_clear_link(0x33);` | Authored link ID removed by that persistent transition; no proper object identity is inferred. |
| `map_restore_floor_state` | 147 | `0x33` | `map_object_pool_clear_link(0x33);` | Authored link ID removed by that persistent transition; no proper object identity is inferred. |
...
| `map_restore_floor_state` | 155 | `0x33` | `map_object_pool_clear_link(0x33);` | Authored link ID removed by that persistent transition; no proper object identity is inferred. |
...
| `map_restore_floor_state` | 177 | `0` | `if (item_stock[0][KF_ITEM_DRAGON_SWORD] != 0 \|\| item_stock[0][KF_ITEM_MOONLIGHT_SWORD] != 0` | Stock row zero and nonzero possession tests for the named Dragon Sword and Moonlight Sword; the script flag supplies the separate alternative. |
| `map_restore_floor_state` | 177 | `0` | `if (item_stock[0][KF_ITEM_DRAGON_SWORD] != 0 \|\| item_stock[0][KF_ITEM_MOONLIGHT_SWORD] != 0` | Stock row zero and nonzero possession tests for the named Dragon Sword and Moonlight Sword; the script flag supplies the separate alternative. |
| `map_restore_floor_state` | 179 | `0x34` | `map_object_pool_clear_link(0x34);` | Authored link ID cleared by either item possession or the transition flag. |
| `map_restore_floor_state` | 182 | `0xd` | `map_object_pool_trigger_link(0xd);` | Authored link ID triggered after completion. |
| `map_restore_floor_state` | 183 | `0` | `actor_pool_begin_death_by_definition(0);` | Authored actor-definition selector in the completion cleanup set {0,2,3,4}; no proper creature identity is inferred. |
| `map_restore_floor_state` | 184 | `2` | `actor_pool_begin_death_by_definition(2);` | Authored actor-definition selector in the completion cleanup set {0,2,3,4}; no proper creature identity is inferred. |
| `map_restore_floor_state` | 185 | `3` | `actor_pool_begin_death_by_definition(3);` | Authored actor-definition selector in the completion cleanup set {0,2,3,4}; no proper creature identity is inferred. |
| `map_restore_floor_state` | 186 | `4` | `actor_pool_begin_death_by_definition(4);` | Authored actor-definition selector in the completion cleanup set {0,2,3,4}; no proper creature identity is inferred. |
| `map_refresh_dialogue_stages` | 198 | `1` | `u16 index = KF_MAP_EVENT_CAPACITY - 1;` | Inclusive countdown starts at capacity minus one; eight slots are visited. |
| `map_refresh_dialogue_stages` | 205 | `0` | `} while (index-- != 0);` | Post-decrement zero endpoint visits the final slot before ending the fixed-size walk. |

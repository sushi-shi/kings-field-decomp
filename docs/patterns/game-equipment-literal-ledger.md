...
after the [equipment domain audit](game-equipment-domains.md) and [shop price follow-up](game-shop-price-domains.md) plus [equipment identity correction](game-item-equipment-identities.md) and [spell identities](game-spell-identities.md). Claim addresses
...
| `menu_spell_select` | 235 | `0` | `s32 input = 0;` | Initial previous input state has no pressed buttons for edge detection. |
| `menu_spell_select` | 237 | `99` | `s32 selection = -99;` | Pending-selection sentinel outside item/spell codes and cancellation -1; retained pending a shared list-result protocol audit. The choice of -99 is unproven. |
...
| `menu_spell_select` | 244 | `10` | `for (code = KF_MAGIC_LIGHTNING_BOLT; code < KF_MAGIC_PLAYER_COUNT; code++, name += 10) {` | Advance one ten-halfword name row per selected spell; the named Lightning Bolt through Light Needle interval excludes the four instant spells. |
...
| `menu_spell_select` | 259 | `1 × 2` | `menu_list_init(&ctx, 1, 1);` | Equipment title bank 1, row 1 for magic; positional layout indices at the shared widget boundary. |
| `menu_spell_select` | 261 | `10` | `ctx.glyphs_per_entry = 10;` | The shared name-row representation has ten glyph halfwords per entry. |
| `menu_spell_select` | 262 | `0 × 2` | `ctx.glyph_rows = &labels[0][0];` | Base address of the first glyph in the first row for the flat list-render API. |
| `menu_spell_select` | 263 | `0` | `ctx.quantities = 0;` | Null quantity list: selection panels display names without stock counts. |
...
| `menu_spell_select` | 293 | `1` | `input = PadRead(1);` | Ignored retail PadRead call-site argument; the linked routine uses its global pad identifier. |
| `menu_spell_select` | 294 | `0` | `if (ctx.entry_count == 0) {` | Empty-list input path, retained even though the current construction appends a none entry. |
| `menu_spell_select` | 295 | `0` | `if (input != 0) {` | Any pressed input acknowledges the empty-list path. |
| `menu_spell_select` | 297, 339 | `1 × 2` | `selection = -1;` | Outer panel cancellation, outside the nonnegative item IDs or row indices. |
| `menu_spell_select` | 299 | `0 × 2` | `} else if ((input & PADLup) != 0 && (prev & PADLup) == 0) {` | Current named button bit set and previous bit clear form a rising edge; zero tests Boolean absence. |
| `menu_spell_select` | 301 | `0` | `if (ctx.selected_index != 0) {` | First list-entry boundary for upward cursor movement. |
| `menu_spell_select` | 303 | `0` | `if (ctx.cursor_row == 0)` | Top visible row: scrolling upward changes the window instead of its cursor row. |
| `menu_spell_select` | 308 | `1` | `ctx.selected_index = ctx.entry_count - 1;` | Wrap to the last zero-based entry. |
| `menu_spell_select` | 310, 329 | `0 × 2` | `ctx.scroll_offset = 0;` | Reset the scroll window to its first entry. |
| `menu_spell_select` | 311 | `1` | `ctx.cursor_row = ctx.entry_count - 1;` | Bottom row of a list shorter than the visible window. |
| `menu_spell_select` | 314 | `1` | `ctx.cursor_row = ctx.visible_rows - 1;` | Last zero-based visible cursor row. |
| `menu_spell_select` | 319 | `0 × 2` | `} else if ((input & PADLdown) != 0 && (prev & PADLdown) == 0) {` | Current named button bit set and previous bit clear form a rising edge; zero tests Boolean absence. |
| `menu_spell_select` | 321 | `1` | `if (ctx.selected_index < ctx.entry_count - 1) {` | Downward movement stops at the last zero-based entry before wrapping. |
| `menu_spell_select` | 323 | `1` | `if (ctx.cursor_row == ctx.visible_rows - 1)` | At the bottom visible row, downward movement scrolls the window. |
| `menu_spell_select` | 328 | `0` | `ctx.selected_index = 0;` | Wrap selection back to the first entry. |
| `menu_spell_select` | 330 | `0` | `ctx.cursor_row = 0;` | Reset the visible cursor to the top row. |
| `menu_spell_select` | 334 | `0 × 2` | `} else if ((input & PADRright) != 0 && (prev & PADRright) == 0) {` | Current named button bit set and previous bit clear form a rising edge; zero tests Boolean absence. |
| `menu_spell_select` | 336 | `1` | `confirm = 1;` | Set the UI confirmation/highlight request on the confirm edge. |
| `menu_spell_select` | 337 | `0 × 2` | `} else if ((input & PADRdown) != 0 && (prev & PADRdown) == 0) {` | Current named button bit set and previous bit clear form a rising edge; zero tests Boolean absence. |
| `menu_spell_select` | 349 | `1` | `if (selection != -1) {` | Apply only a completed selection; outer cancellation leaves equipment or magic unchanged. |

...
| `lhu v1,2(a0)` at 800158b8/800158cc added twice into `cutting_defense` | two `+=` statements of the same field, as retail does for every armor slot | `player_recalculate_combat_stats` |
...
| retail frame 224 with `direction` at sp+40 (ours 216, sp+32) | all observed local-object offsets differ by eight bytes, but these accesses do not establish another source object or its extent. The frame difference remains unattributed; an unreferenced placeholder would be artificial padding | same |

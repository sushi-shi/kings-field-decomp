...
## Integration with current data owners and Boolean modeling

The integration is verified against master `273accaa`, including its Sony
helpers, repaired allocator/map-object owners and Boolean interfaces. The
new owner fields and private save-event declarations are preserved; the
newly added layout assertions are removed with the earlier assertions.
The combined inventory contains 129 structures, 871 fields and 783 named
fields. Compiler-based layout controls remain in the tests.

A fresh master build and the combined build both retain **453/471 exact game
functions** and **13/13 exact vendored controls**. All 484 compiled functions
were compared individually: 483 instruction listings and scores are unchanged.
The sole difference remains the glyph cursor change described above in GAME
`menu_draw_item_detail` (`97.814210%` to `97.808750%`), with identical effective
write addresses. All 112 objects retain their data bytes, extents and ordered
relocation targets, including the repaired owners.

The integration passes 769 repository tests with nine skips, all 112 modern
type-checking variants, Ruff and whitespace checks. Full `kf build` and
`kf check` retain the baseline's 23 diverging data-owning units and incomplete
ownership/relink gates. Their check logs differ only in changed-source ledger
counts; no new failure or lost exact is introduced. The match ledger is
unchanged. Detailed baseline and candidate logs and individual verdicts are
under `build/cleanup-integration/` in the integration worktree.

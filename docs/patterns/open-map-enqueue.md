...
## Exact closure: header declaration lifetime (`codex/open-10-functions`)
...
The focused hypothesis declares the packet header before constructing the
normal-stream pointer. This reflects their source roles without changing a
type, expression, lifetime, call, branch, referent or adding storage. A
TU-only `kf try` produced an identical listing; strict `kf match` confirms
**100.000000%**, with all 952 bytes and relocations exact. OPEN moved from
98/108 to 99/108 exact, and only this function was banked.

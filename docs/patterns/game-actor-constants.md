...
| Dispatcher drift | The acceleration 1, per-axis speed limit 100 and yaw speed limit 32 are now named in the [motion-limit review](game-actor-motion-limits.md). Yaw `++`/`--` still advances its increment by one angle unit; zeros reset motion and phase. The RNG half-domain arithmetic retains its existing formula. |
...
| Dispatcher return-home | The exclusive per-axis tolerance 200 is now named in the [motion-limit review](game-actor-motion-limits.md). Raw heading-change cutoff 2048 accepts SDK RNG values 0..2047; why this reconsideration rate was chosen is unresolved. Phase `4b0` hex (1200) is an authored reset sample in the existing mixed-slot path, not a uniform cadence. Preserve the uninitialized coordinate read and the differing move/melee animation accesses. |

...
| `render_bind_animated_instance` | 175 | `1` | `copy_vertices(&tmd_morph_scratch[1], record->cached_vertices, vertex_count);` | Published vertices start after the one leading SDK vector temporarily used by the rest-morph header blend. |
...
| `render_bind_animated_instance` | 189 | `1` | `tmd_set_current_vertices(&tmd_morph_scratch[1]);` | Publish vertices after the extra leading vector; the same one-vector displacement is used by the initial scratch copy. |

...
All **127 retained occurrences** in **92 expression groups** have explicit reasons.
...
| `menu_magic_panel` | 33 | `0` | `s32 confirm = 0;` | Initially no pending confirmation; this is a UI highlight/request flag, not an equipment category. |
| `menu_magic_panel` | 34 | `0` | `s32 input = 0;` | Initial previous input state has no pressed buttons for edge detection. |
| `menu_magic_panel` | 36 | `99` | `s32 selection = -99;` | Pending-selection sentinel outside item/spell codes and cancellation -1; retained pending a shared list-result protocol audit. The choice of -99 is unproven. |
...
| `menu_magic_panel` | 70 | `0` | `KF_MENU_PREVIEW_MAGIC_ICON, codes[ctx.selected_index], 0, KF_ITEM_PRICE_BUY)` | The shop/detail index is an unused zero for the magic-icon confirmation path. |
| `menu_magic_panel` | 74 | `99` | `if (selection != -99) {` | Any completed selection or cancellation ends the modal loop; pending -99 continues it. |
| `menu_magic_panel` | 81 | `0` | `confirm = 0;` | Clear the pending confirmation/highlight request for the next input frame. |
| `menu_magic_panel` | 83 | `1` | `input = PadRead(1);` | Ignored retail PadRead call-site argument; the linked routine uses its global pad identifier. |
| `menu_magic_panel` | 84 | `0` | `if (ctx.entry_count == 0) {` | Empty-list input path, retained even though the current construction appends a none entry. |
| `menu_magic_panel` | 85 | `0` | `if (input != 0) {` | Any pressed input acknowledges the empty-list path. |
| `menu_magic_panel` | 87, 127 | `1 × 2` | `selection = -1;` | Outer panel cancellation, outside the nonnegative item IDs or row indices. |
| `menu_magic_panel` | 89 | `0 × 2` | `} else if ((input & PADLup) != 0 && (prev & PADLup) == 0) {` | Named button present now and absent previously gives a rising edge; zero means no corresponding bit. |
| `menu_magic_panel` | 91 | `0` | `if (ctx.selected_index != 0) {` | First list-entry boundary for upward cursor movement. |
| `menu_magic_panel` | 93 | `0` | `if (ctx.cursor_row != 0)` | A nonzero visible cursor can move upward without scrolling the list. |
| `menu_magic_panel` | 98 | `1` | `ctx.selected_index = ctx.entry_count - 1;` | Wrap to the last zero-based entry. |
| `menu_magic_panel` | 99, 117 | `0 × 2` | `ctx.scroll_offset = 0;` | Reset the scroll window to its first entry. |
...
| `menu_magic_panel` | 107 | `0 × 2` | `} else if ((input & PADLdown) != 0 && (prev & PADLdown) == 0) {` | Named button present now and absent previously gives a rising edge; zero means no corresponding bit. |
| `menu_magic_panel` | 109 | `1` | `if (ctx.selected_index < ctx.entry_count - 1) {` | Downward movement stops at the last zero-based entry before wrapping. |
| `menu_magic_panel` | 111 | `1` | `if (ctx.cursor_row != ctx.visible_rows - 1)` | Last zero-based visible row is the downward scrolling boundary. |
| `menu_magic_panel` | 116 | `0` | `ctx.selected_index = 0;` | Wrap selection back to the first entry. |
| `menu_magic_panel` | 118 | `0` | `ctx.cursor_row = 0;` | Reset the visible cursor to the top row. |
| `menu_magic_panel` | 122 | `0 × 2` | `} else if ((input & PADRright) != 0 && (prev & PADRright) == 0) {` | Named button present now and absent previously gives a rising edge; zero means no corresponding bit. |
| `menu_magic_panel` | 124 | `1` | `confirm = 1;` | Set the UI confirmation/highlight request on the confirm edge. |
| `menu_magic_panel` | 125 | `0 × 2` | `} else if ((input & PADRdown) != 0 && (prev & PADRdown) == 0) {` | Named button present now and absent previously gives a rising edge; zero means no corresponding bit. |
| `menu_magic_panel` | 130 | `0` | `if (ctx.entry_count != 0)` | Only preview an item when the list has entries. |
| `menu_magic_panel` | 135 | `1` | `if (selection == -1)` | Return the outer-panel cancellation before indexing or debiting a spell record. |
...
| `menu_option_root` | 165 | `0` | `s32 cursor = 0;` | Start the equipment menu at its first row (weapon); this integer tracks UI position. |
| `menu_option_root` | 166 | `0` | `s32 confirm = 0;` | Initially no pending confirmation; this is a UI highlight/request flag, not an equipment category. |
| `menu_option_root` | 167 | `0` | `s32 input = 0;` | Initial previous input state has no pressed buttons for edge detection. |
| `menu_option_root` | 169 | `99` | `s32 result = -99;` | Pending outer-menu result outside completed cancellation -1; original choice of -99 unknown. |
| `menu_option_root` | 170 | `1` | `s32 selection = -1;` | Initially no equipment subpanel selection is pending. |
| `menu_option_root` | 174 | `1, 9, 0 × 2` | `menu_draw_window(1, 9, 0, 0);` | Retail equipment layout1 contains nine rows: eight equipment/magic categories and return. Start at cursor zero without a confirmation highlight. |
| `menu_option_root` | 178 | `1` | `if (selection != -1 \|\| result == selection) {` | Redraw after a completed subpanel selection or the matching -1 outer result; retain the two-result protocol. |
| `menu_option_root` | 181, 236 | `1 × 2, 9 × 2` | `menu_draw_window(1, 9, cursor, confirm);` | Same authored equipment layout1 and complete nine-row extent; this UI row count is independent of the coincidentally nine spell labels. |
| `menu_option_root` | 183 | `1, 0` | `while (PadRead(1) != 0)` | Preserve the ignored PadRead call-site argument 1 and wait until the returned button bits are zero; the linked SDK uses global PadIdentifier, not this argument as a port. |
| `menu_option_root` | 205 | `1` | `selection = -1;` | Clear the completed subpanel selection before reading the next equipment-menu input. |
| `menu_option_root` | 206 | `99` | `if (result != -99)` | A completed outer-menu result ends the modal loop; -99 remains pending. |
| `menu_option_root` | 209 | `0` | `confirm = 0;` | Clear the pending confirmation/highlight request for the next input frame. |
| `menu_option_root` | 211 | `1` | `input = PadRead(1);` | Ignored retail PadRead call-site argument; the linked routine uses its global pad identifier. |
| `menu_option_root` | 212 | `0 × 2` | `if ((input & PADLup) != 0 && (prev & PADLup) == 0) {` | Named button present now and absent previously gives a rising edge; zero means no corresponding bit. |
| `menu_option_root` | 214 | `0` | `if (cursor != 0)` | First-row boundary for upward movement. |
| `menu_option_root` | 217 | `8` | `cursor = 8;` | Wrap upward to zero-based return row8, the final row in the nine-row equipment layout. |
| `menu_option_root` | 218 | `0 × 2` | `} else if ((input & PADLdown) != 0 && (prev & PADLdown) == 0) {` | Named button present now and absent previously gives a rising edge; zero means no corresponding bit. |
| `menu_option_root` | 220 | `8` | `if (cursor != 8)` | Return row8 is the final-row boundary for downward movement. |
| `menu_option_root` | 223 | `0` | `cursor = 0;` | Wrap downward to the first equipment row. |
| `menu_option_root` | 224 | `0 × 2` | `} else if ((input & PADRright) != 0 && (prev & PADRright) == 0) {` | Named button present now and absent previously gives a rising edge; zero means no corresponding bit. |
| `menu_option_root` | 226 | `1` | `confirm = 1;` | Set the UI confirmation/highlight request on the confirm edge. |
| `menu_option_root` | 227 | `8` | `if (cursor < 8)` | Rows0..7 dispatch categories; row8 completes the outer menu instead. |
| `menu_option_root` | 230, 233 | `1 × 2` | `result = -1;` | Complete the outer menu on return-row confirmation or the cancel edge. |
| `menu_option_root` | 231 | `0 × 2` | `} else if ((input & PADRdown) != 0 && (prev & PADRdown) == 0) {` | Named button present now and absent previously gives a rising edge; zero means no corresponding bit. |

...
the executable proves the numbered component lanes and status bits, but does
not preserve their original source or Japanese lore names.
...
The following BSS identities are individually supported. They remain
`scope=unknown`: linked references prove shared storage and purpose, but cannot
distinguish original external globals from file-local statics. Contiguity alone
is not yet used to claim one source struct.

| VA | Identity | Type | Evidence |
| --- | --- | --- | --- |
| `0x800a07aa` | `player_status_effect_flags` | `u16` | Damage sets bits 0..3; the player update clears each bit when its timer expires. |
| `0x800a07bc` | `player_damage_defense_component0` | `u16` | First defense supplied to the component formula. |
| `0x800a07be` | `player_damage_defense_component1` | `u16` | Second defense supplied to the component formula. |
| `0x800a07c0` | `player_damage_defense_component2` | `u16` | Third defense supplied to the component formula. |
| `0x800a07c2` | `player_status_effect2_resistance` | `u16` | Random threshold checked before status bit 2 is accepted. |
| `0x800a07c4` | `player_damage_defense_component3` | `u16` | Fourth defense supplied to the component formula. |
| `0x800a07c6` | `player_damage_defense_component4` | `u16` | Fifth defense supplied to the component formula. |
| `0x800a07c8` | `player_status_effect0_timer` | `s16` | Initialized to 600 and decremented for bit 0. |
| `0x800a07ca` | `player_status_effect1_timer` | `s16` | Uses `-1` as inactive and is extended toward 970/1000 for bit 1. |
| `0x800a07cc` | `player_status_effect2_timer` | `s16` | Initialized to 600 after the random resistance check. |
| `0x800a07ce` | `player_status_effect3_timer` | `s16` | Initialized to 300 and decremented for bit 3. |
...
their family. The exact component elements and status effects remain numbered
until strings, UI assets, the manual, or another direct witness establishes
their names. The shared base-power halfword at `0x800a07a6`, the equipment byte
at `0x800a0815`, the player action-state owner at `0x800a0820`, and the vertical
reference at `0x800a0834` likewise remain address-derived in this pass.

//! `STAT.DAT` database split used by retail `GAME.EXE`.
...
pub trait ItemFileSearch {
    /// Models `CdSearchFile`. `path` is the full 40-byte, zero-filled C local;
    /// `record` is the caller-initialized 20-byte game table entry.
    fn search_file(
        &mut self,
        index: u8,
...
/// Applies all six retail memcpy extents. All sizes are validated before the
/// first write, so malformed test inputs cannot leave a mixed database.
...
/// Resolves all 80 item model names and applies the retail whole-sector size
/// rounding to successful 20-byte `KfCdFileEntry` records. Failed searches leave the
/// caller's record untouched even if a test callback modifies its candidate.
...
/// Complete `item_load_database` data transformation. File-table capacity is
/// checked before any STAT bank is written.

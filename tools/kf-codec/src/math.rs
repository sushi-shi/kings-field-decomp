//! Shared decoded game and SDK vector types.
//!
//! These are values, not host-layout views of PS1 memory. Explicit byte
//! conversions preserve signed components and SDK padding on every host.

/// The game's six-byte `KfVec3s` (no SDK padding).
...
/// Three decoded world coordinates; excludes the SDK `VECTOR` pad word.
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct Vec3i {
    pub x: i32,
    pub y: i32,
...
/// The game's `KfEulerAngles`, whose components use 4096 units per turn.
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
...
/// SDK `SVECTOR`, including the stored pad halfword.
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct SVector {
    pub vx: i16,
    pub vy: i16,

//! Composition of the MIXA/MIXB outer walk performed by `map_resources_load`.
//!
//! Inner TMD, VAB, placement, and asset-archive payloads are deliberately
//! emitted through typed callback boundaries.  Their dedicated codecs own
//! those grammars; this module owns chunk order, five-grid copying, variant
//! selection, and callback order.
...
impl<'a> MapResources<'a> {
    ...
    pub fn execute(
        ...
        callbacks.event(MapResourceEvent::RetainMixAAllocation {
            // `block` is the VAB-body chunk header after the first
            // STREAM_NEXT in the original expression, not the file base.

//! Typed top-level views of the GAME resource streams.
//!
//! These views name only chunk ordering proved by the retail consumers. Their
//! payloads remain byte slices: several GAME loaders deliberately read across
//! a chunk boundary, so treating every payload as a self-contained typed array
//! would describe a stricter format than the executable actually consumes.
...
/// COM.DAT's seven chunks in the order consumed by `common_resources_load`.
...
pub struct CommonData<'a> {
    ...
    /// The exact 1,176-byte source copied by retail, including the following
    /// magic chunk's four-byte length and its first 416 payload bytes.
    pub armor_records_source: &'a [u8],
    pub magic_records: &'a [u8],
    pub map_object_definitions: &'a [u8],
    /// The exact 1,280-byte source copied by retail, including the following
    /// growth chunk's four-byte length and its first 148 payload bytes.
...
pub struct MapGrids<'a> {
    ...
    pub collision: &'a [u8],
    /// Bytes after the five 100-by-100 grids in the same chunk.
    pub remainder: &'a [u8],
}
...
/// MIXA.DAT's eight chunks in the order consumed by `map_resources_load`.
#[derive(Debug, Clone, Copy)]
pub struct MixA<'a> {
    pub vab_header: &'a [u8],
    pub vab_body: &'a [u8],
...
/// MIXB.DAT's TMD chunks and nested asset archives.
///
/// Floors one through four contain the optional final embedded-variant archive.
/// Floor five omits it and loads CHR1.MIM through CHR3.MIM separately.
#[derive(Debug, Clone, Copy)]
pub struct MixB<'a> {

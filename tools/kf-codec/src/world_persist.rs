//! Serialization of one live floor into GAME's nested world-state record.
//!
//! This models `GAME.EXE` `map_world_state_persist` (0x80035b5c). Callers
//! provide the exact writable span for the selected floor: normally 1,700
//! bytes, but only 1,690 bytes remain after floor five's record start in the
//! saved 8,500-byte world block. Bytes after the encoded prefix are retained.
...
/// Serialize the live pools into the variable-length prefix used by retail.
///
/// Every input extent, derived index, and the final output size is preflighted
/// before the first write. This gives the safe Rust boundary transactional
/// behavior; retail trusts these runtime structures and has no bounds checks.

//! Sony VAB banks and SEQ event streams consumed by GAME's audio subsystem.
//!
//! The fixed structures follow the supplied Release 2.5 `LIBSND.H` types
//! (`VabHdr`, `ProgAtr`, and `VagAtr`).  VAB header words are little-endian;
...
/// Size of one Sony `SeqStruct` track record in this Release 2.5 runtime.
pub const SEQ_SCORE_RECORD_SIZE: usize = 176;
/// Size of GAME's caller-owned `KfAudioState` aggregate.
...
impl<'a> VabBank<'a> {
    ...
    /// Read one row from the fixed 128-entry `ProgAtr` table.
    ///
    /// `VabHdr.ps` sizes the following dense tone table; it does not truncate
    /// this table.  Sony's open-head routine scans and mutates all 128 rows,
    /// including rows beyond `ps` in retail banks.
    ...
    /// Iterate the complete fixed 128-entry `ProgAtr` table.
...
pub struct SeqHeader {
    ...
    pub version: u32,
    pub resolution: u16,
    /// Initial tempo in microseconds per quarter note (24-bit file field).
    pub tempo: u32,
...
/// Observable sound-library service emitted while applying one SEQ event.
///
/// These values are kept typed rather than stored as raw register words.  The
/// caller decides how to deliver them to the SPU backend; the parser itself is
/// allocation-free and has no platform dependencies.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// Initialize the complete caller-owned Release 2.5 score record.
///
/// Only fields written by `InitSoundSeq` are changed; bytes unknown to that
/// initializer remain untouched.  File pointers are represented as the MIPS
/// virtual address at which `sequence` will be mapped by the caller.
...
/// Apply the successful GAME/Sony sequence-open state transition.
///
/// `allocated_sequence_id` is the explicit provider result from `SsSeqOpen`.
/// The function mirrors the provider's open-bit mutation and GAME's two
/// caller-owned stores while leaving the sequence buffer and every unrelated
/// byte untouched.
...
/// Apply the score-record mutations and return the ordered SPU effect for one
/// decoded event.  This models the event families present in King's Field's
/// complete shipped SEQ corpus.  Unsupported Sony controller/meta families
/// are reported explicitly instead of being silently normalized.
